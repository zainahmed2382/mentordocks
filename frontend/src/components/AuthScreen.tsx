...
export function AuthScreen({ onClose, onLoginSuccess }: AuthScreenProps) {
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-black/75 backdrop-blur-md flex items-center justify-center p-4">
      {/* Container Card */}
      <div className="w-full max-w-md bg-background border border-white/10/90 rounded-2xl p-6 md:p-8 relative shadow-2xl overflow-hidden">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-80 h-32 bg-primary/10 blur-[50px] pointer-events-none rounded-full" />

        <div className="flex justify-between items-center relative z-10 mb-6">
          <div className="flex items-center gap-1.5">
            <Sparkles className="h-4.5 w-4.5 text-blue-400" />
            <span className="text-[10px] font-mono font-bold text-blue-400 uppercase tracking-widest bg-primary/10 px-2 py-0.5 rounded">
              Mentor Docks Account Gate
            </span>
          </div>
          <button 
            onClick={onClose}
            className="text-secondary hover:text-primary p-1 hover:bg-white/5 rounded-lg transition overflow-hidden h-7 w-7 flex items-center justify-center cursor-pointer"
          >
        ...
        {success ? (
          <div className="text-center py-8 space-y-4 relative z-10 animate-fade-in">
            ...
            <div className="w-20 bg-surface h-1 mx-auto rounded-full overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-left">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-surface border border-white/10/80 rounded-xl py-2.5 pl-10 pr-4 text-xs focus:outline-none focus:border-blue-500 text-white placeholder-zinc-500"
                    />
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-mono text-secondary uppercase tracking-wider">Password</label>
                <div className="relative">
              ...
              {isSignUp && (
                <div className="space-y-1">
                  <label className="text-[10px] font-mono text-secondary uppercase tracking-wider">Professional Role Focus</label>
                  <div className="relative">
                    <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-secondary pointer-events-none" />
                    <select
                      value={role}
                      onChange={(e) => setRole(e.target.value as UserProfile["role"])}
                      className="w-full bg-surface border border-white/10/80 rounded-xl py-2.5 pl-10 pr-4 text-xs focus:outline-none focus:border-blue-500 text-primary appearance-none font-sans"
                    >
              ...
              <button
                type="submit"
                className="w-full py-3 bg-background text-primary font-bold rounded-xl text-xs uppercase tracking-wider transition hover:bg-white/5 mt-2 cursor-pointer"
              >
            ...
            <div className="flex items-center justify-center gap-1.5 text-[10px] text-secondary pt-2 border-t border-white/10">
