...
export function SplashScreen({ onDismiss }: SplashScreenProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-background flex flex-col items-center justify-between p-6 overflow-hidden md:p-12">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[550px] h-[550px] rounded-full bg-primary/10 blur-[120px] pointer-events-none -z-10 animate-pulse" />

      {/* Top Section */}
      <div className="w-full flex justify-between items-center max-w-5xl">
        <div className="flex items-center gap-2">
          <Zap className="h-5 w-5 text-accent animate-pulse" />
          <span className="text-xs font-mono text-secondary tracking-widest uppercase">System Core Boot</span>
        </div>
        <button
          onClick={onDismiss}
          className="text-xs font-mono text-secondary hover:text-primary bg-surface border border-white/10 hover:border-white/10 px-3 py-1 rounded transition cursor-pointer"
        >
          Skip Boot [Esc]
        </button>
      </div>
      ...
      <div className="flex flex-col items-center text-center space-y-6 max-w-md my-auto">
        ...
        <div className="relative mb-2">
          <div className="absolute inset-0 rounded-full border border-accent/30 animate-ping opacity-60" />
        ...
        <div className="w-full bg-background border border-white/10/80 rounded-xl p-3 text-left font-mono text-[11px] h-12 flex items-center gap-2.5 overflow-hidden shadow-inner text-secondary">
          <Terminal className="h-4 w-4 text-accent shrink-0 animate-pulse" />
          <span className="truncate">{BOOT_LOGS[statusIdx]}</span>
        </div>

        {/* Big visual progress percentage bar */}
        <div className="w-full space-y-2">
          <div className="flex justify-between items-center font-mono text-[10px] text-secondary px-1">
