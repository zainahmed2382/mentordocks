...
const STEPS = [
  {
    ...
    icon: <Cpu className="h-5 w-5 text-custom-ochre animate-pulse" />,
  },
  {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[450px] p-8 max-w-xl mx-auto text-center">
      ...
      <div className="relative mb-8">
        {/* Ambient Ring Glow */}
        <div className="absolute inset-0 rounded-full bg-custom-ochre/10 blur-xl opacity-50 animate-ping -z-10" />

        {/* Outer Tech Ring */}
        <div className="w-24 h-24 rounded-full border-4 border-custom-choco/20 border-t-white animate-spin flex items-center justify-center">
          {/* Inner pulsating core */}
          <div className="w-16 h-16 rounded-full bg-custom-cream border border-custom-choco/20 flex items-center justify-center">
      ...
      <div className="bg-custom-sand border border-custom-choco/20 px-3 py-1 rounded-full text-xs font-mono text-custom-choco/80 mb-6 max-w-sm truncate shadow-inner">
        Crawling: {url}
      </div>
      ...
      <div className="space-y-2 mb-8">
        ...
        <p className="text-sm text-custom-choco/80 max-w-md mx-auto leading-relaxed h-10">
          {STEPS[currentStepIndex].subtext}
        </p>
      </div>

      {/* Progress Bar Container */}
      <div className="w-full bg-custom-sand h-2.5 rounded-full mb-3 overflow-hidden border border-custom-choco/20 shadow-inner">
        <div
          className="bg-gradient-to-r from-blue-500 to-indigo-500 h-full rounded-full transition-all duration-300 ease-out"
          style={{ width: `${percent}%` }}
        />
      </div>
      <span className="text-xs font-mono font-bold text-custom-choco/60">{percent}% Audited</span>

      {/* Passive Step indicators */}
      <div className="mt-8 pt-6 border-t border-custom-choco/20 w-full">
                ...
                <div
                  className={`h-6 w-6 rounded-full border flex items-center justify-center text-xs ${
                    isCompleted
                      ...
                      : "bg-custom-sand border-custom-choco/20 text-[#555]"
                  }`}
                >
                  {isCompleted ? "✓" : idx + 1}
                </div>
                <span className="text-xs text-custom-choco">{step.message}</span>
              </div>
            );
          })}
        </div>
