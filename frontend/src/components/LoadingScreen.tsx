  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[450px] p-8 max-w-xl mx-auto text-center">
      ...
      <div className="relative mb-8">
        ...
        {/* Outer Tech Ring */}
        <div className="w-24 h-24 rounded-full border-4 border-slate-200 border-t-white animate-spin flex items-center justify-center">
        ...
        {/* Tiny floaters */}
        <div className="absolute -top-1 -right-1 p-1 bg-slate-50 rounded-full border border-slate-200">
          <Sparkles className="h-3.5 w-3.5 text-amber-400 animate-pulse" />
        </div>
      </div>

      {/* Scanned URL indicator */}
      <div className="bg-slate-50 border border-slate-200 px-3 py-1 rounded-full text-xs font-mono text-slate-600 mb-6 max-w-sm truncate shadow-inner">
        Crawling: {url}
      </div>
      ...
      <div className="space-y-2 mb-8">
        ...
        <p className="text-sm text-slate-600 max-w-md mx-auto leading-relaxed h-10">
          {STEPS[currentStepIndex].subtext}
        </p>
      </div>

      {/* Progress Bar Container */}
      <div className="w-full bg-slate-50 h-2.5 rounded-full mb-3 overflow-hidden border border-slate-200 shadow-inner">
        <div
          className="bg-gradient-to-r from-blue-500 to-indigo-500 h-full rounded-full transition-all duration-300 ease-out"
          style={{ width: `${percent}%` }}
        />
      </div>
      <span className="text-xs font-mono font-bold text-slate-500">{percent}% Audited</span>
                ...
                <div
                  className={`h-6 w-6 rounded-full border flex items-center justify-center text-xs ${
                    isCompleted
                      ...
                      : "bg-slate-50 border-slate-200 text-[#555]"
                  }`}
                >
                  {isCompleted ? "✓" : idx + 1}
                </div>
                <span className="text-xs text-slate-900">{step.message}</span>
              </div>
            );
          })}
        </div>
